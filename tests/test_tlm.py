...
class TestUnitTlm:
    @staticmethod
    def tlm_ref(ell: int, m: int, I_val: float, m1: float, m2: float) -> float:
        """
        Reference implementation of <I,m1| T_{l,m} |I,m2> using the Wigner-Eckart
        theorem (SI § S2), via SymPy's wigner_3j.
        """
        # Reduced matrix element ⟨I‖T^(ell)‖I⟩ from Eq. S.62
        num = math.factorial(ell) * math.factorial(ell) * math.factorial(int(2 * I_val + ell + 1))
        den = (2**ell) * math.factorial(2 * ell) * math.factorial(int(2 * I_val - ell))
        red = math.sqrt(num / den)

        # Prepare rational spins for wigner_3j
        j = Rational(int(2 * I_val), 2)
        mj1 = Rational(-int(2 * m1), 2)
        mj = Rational(int(2 * m), 2)
        mj2 = Rational(int(2 * m2), 2)

        threej = wigner_3j(j, ell, j, mj1, mj, mj2)
        phase = (-1) ** int(I_val - m1)
