...
from spinOps import DLM, Rotate, create_rho1, create_rho2, wigner_d
...
class TestUnitWigner_d:
    ...
    def test_unitarity(self):
        ...
        for l, beta in [(1, math.pi / 5), (2, math.pi / 3)]:
            ...
            mat = np.array(
                [
                    [wigner_d(l, m1, m2, beta) for m2 in range(-l, l + 1)]
                    for m1 in range(-l, l + 1)
                ],
                dtype=complex,
            )
            ...
            assert np.allclose(
                prod, np.eye(size), atol=1e-8
            ), f"Unitarity failed for l={l}, beta={beta}"
...
class TestDLM:
    ...
    def test_unitarity_of_DLM(self):
        ...
        for l, alpha, beta, gamma in [(1.0, 0.3, 0.4, -0.5), (2.0, -0.8, 1.0, 0.9)]:
            ...
            assert np.allclose(
                prod, np.eye(size), atol=1e-8
            ), f"DLM unitarity failed for l={l}"
